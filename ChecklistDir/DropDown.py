    ...
    def __init__(self, master, parent):
        ...
        def show(line):
            ...
            if line != 0 and line <= len(todo):
                myLablel = Label(self.master,
                                 text="Zadanie " + str(line) + " z " + str(len(todo)) + ":\n" + todo[line - 1],
                                 fg="Lightgrey").grid(row=config.line, column=0, columnspan=2)
            if line < len(todo):
                myLablel2 = Label(self.master,
                                  text="Zadanie " + str(line + 1) + " z " + str(len(todo)) + ":\n" + todo[line],
                                  fg="Magenta").grid(row=config.line + 1, column=0, columnspan=2)
            if line == len(todo):
                myLablel3 = Label(self.master, text="Gratulacje zrobiłeś wszystko", fg="Gold", font=" bold",
                                  bg="black").grid(row=config.line + 2, column=0, columnspan=2)
            config.line += 1
